...
namespace SecurityLibrary.AES
{
    ...
    public class AES : CryptographicTechnique
    {
        ...
        string[,] mixColumn(int[,] A, string[,] B)
        {
            string x1B = "00011011";
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int k = 0; k < 4; k++)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        string B_jk = Convert.ToString(Convert.ToInt64(B[j, k], 16), 2);//Convert Hex to Bin
                        B_jk = missingZeros(B_jk, 8);
                        string tmp = B_jk;
                        if (A[i, j] >= 2)
                        {
                            tmp = shiftLeft(tmp);
            ...
            for (int row = 0; row < 4; row++)
            {
                for (int col = 0; col < 4; col++)
                {
                    C[row, col] = HexConverted(C[row, col]);
                }
            }
            return C;
        }
